...
Installed package check after testing:
...
`com.termux` was deliberately preserved. A legacy pre-rename package was also
present on the phone during migration testing, but the active package under test
was `org.hedgeyos`. hedgeyos independence was verified from the package
identity, APK contents, embedded X11 startup path, bundled PRoot and rootfs
paths, and absence of a separate `com.termux.x11` package or VNC/RDP APK
payload.
...
| First boot uses bundled rootfs/proot assets | Pass for APK structure; device runtime previously extracted installed rootfs | APK inspection confirms bundled Debian rootfs and PRoot assets. |
...
| Installed Termux app preserved | Pass | `com.termux` remained installed after testing. |
...
| APT package install proof on exact published alpha | Not rerun for this published alpha | The rootfs includes Debian APT sources; final release should record an `apt update` and package install log against the exact artifact. |
| Clean target without preinstalled Termux/VNC packages | Not complete for final release | Independence is shown by code/APK/process evidence, but final `v0.1.0` should include a clean-profile/device pass. |

## Release Decision

`v0.1.0-alpha.1` is suitable as a published prerelease/test APK. Do not tag final
`v0.1.0` yet; final release still needs production signing and the remaining
acceptance gaps above recorded against the exact final artifact.
